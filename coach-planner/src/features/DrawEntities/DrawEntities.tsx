...
export const DrawEntities = ({ width, height }: { width: number; height: number }) => {
  ...
  const players = useAppSelector(selectPlayersObject);

  return (
    <>
      {image && <Image image={image} width={width} height={height} />}
      {lines.length > 0 &&
        lines.map((line) => <LineComponent current={current} line={line} key={line.id} />)}
      {equipment.length > 0 && equipment.map((elem) => selectEquipment(elem))}
      {players.length > 0 &&
        players.map((player) => <PlayerComponent player={player} key={player.id} />)}
    </>
  );
};
