...
export const InputDate = ({
  ...
  const setEndDateTime = (date: string, time: string, duration: number) => {
    ...
    return new Date(
      new Date(selectedDate).getTime() + duration * millisecond * secondInMinute
    ).toISOString();
  };
  ...
  const onChangeTime = (
    time: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>
  ) => {
  ...
  const onChangeDuration = (
    duration: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>
  ) => {
    setDuration(Number(duration.target.value));
    setValue(
      "endAt",
      setEndDateTime(date, time, Number(duration.target.value))
    );
  };
